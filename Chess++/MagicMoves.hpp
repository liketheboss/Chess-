 ...
 *
 *Unsigned 64 bit integers are referenced by this generator as Bitboard.
...
#define USE_INLINING /*the MMINLINE keyword is assumed to be available*/
	...
	#define __64_BIT_INTEGER_DEFINED__
		...
		typedef unsigned __int64 Bitboard; //For the old microsoft compilers
	#else
		typedef unsigned long long  Bitboard; //Supported by MSC 13.00+ and C99
...
#ifndef C64
	#if (!defined(_MSC_VER) || _MSC_VER>1300)
		#define C64(constantBitboard) constantBitboard##ULL
	#else
		#define C64(constantBitboard) constantBitboard
	#endif
#endif
...
extern const unsigned int magicmoves_r_shift[64];
...
#ifdef USE_INLINING
	static MMINLINE Bitboard Bmagic(const unsigned int square,const Bitboard occupancy)
	{
		#ifndef PERFECT_MAGIC_HASH
			#ifdef MINIMIZE_MAGIC
	...
	static MMINLINE Bitboard Rmagic(const unsigned int square,const Bitboard occupancy)
	{
		#ifndef PERFECT_MAGIC_HASH
			#ifdef MINIMIZE_MAGIC
	...
	static MMINLINE Bitboard BmagicNOMASK(const unsigned int square,const Bitboard occupancy)
	{
		#ifndef PERFECT_MAGIC_HASH
			#ifdef MINIMIZE_MAGIC
	...
	static MMINLINE Bitboard RmagicNOMASK(const unsigned int square, const Bitboard occupancy)
	{
		#ifndef PERFECT_MAGIC_HASH
			#ifdef MINIMIZE_MAGIC
	...
	static MMINLINE Bitboard Qmagic(const unsigned int square,const Bitboard occupancy)
	{
		return Bmagic(square,occupancy)|Rmagic(square,occupancy);
	}
	static MMINLINE Bitboard QmagicNOMASK(const unsigned int square, const Bitboard occupancy)
	{
