...
const Bitboard* magicmoves_b_indices[64]=
{
	...
	magicmovesbdb+1632, magicmovesbdb+2272, magicmovesbdb+4896, magicmovesbdb+5184
		...
		Bitboard magicmovesbdb[64][1<<9];
	#else
		Bitboard magicmovesbdb[1428];
...
#else
	#ifndef PERFECT_MAGIC_HASH
		Bitboard magicmovesrdb[64][1<<12];
	#else
		Bitboard magicmovesrdb[4900];
	...
	for(i=0;i<64;i++)
	{
		...
		for(temp=0;temp<(((Bitboard)(1))<<numsquares);temp++)
		{
			Bitboard tempocc=initmagicmoves_occ(squares,numsquares,temp);
				...
				Bitboard index=(((tempocc)*magicmoves_b_magics[i])>>MINIMAL_B_BITS_SHIFT);
				int j;
				for(j=0;j<1428;j++)
				{
					if(!magicmovesbdb[j])
	...
	for(i=0;i<64;i++)
	{
		...
		for(temp=0;temp<(((Bitboard)(1))<<numsquares);temp++)
		{
			Bitboard tempocc=initmagicmoves_occ(squares,numsquares,temp);
				...
				Bitboard index=(((tempocc)*magicmoves_r_magics[i])>>MINIMAL_R_BITS_SHIFT);
				int j;
				for(j=0;j<4900;j++)
				{
					if(!magicmovesrdb[j])
